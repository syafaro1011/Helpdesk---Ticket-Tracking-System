    ...
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">Perbarui detail laporan kendala atau lampiran foto sebelum
                    ditangani oleh teknisi</p>
            </div>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-6xl space-y-6 sm:px-6 lg:px-8"
            x-data="{
                ...
                removeAttachment: {{ old('remove_attachment') ? 'true' : 'false' }},
                handleFile(event) {
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3 lg:items-start">
                        ...
                        <form action="{{ route('user.tickets.update', $ticket->id) }}" method="POST"
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="5" required
                                    class="shadow-2xs block w-full rounded-lg border-gray-300 p-3 text-sm focus:border-sky-500 focus:ring-sky-500">{{ old('description', $ticket->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-xs font-medium text-rose-500">{{ $message }}</p>
                                @enderror
                            </div>
                                ...
                                @if ($ticket->attachment && \Illuminate\Support\Facades\Storage::disk('public')->exists($ticket->attachment))
                                    <div class="mb-3 flex items-center gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
                                        <img src="{{ \Illuminate\Support\Facades\Storage::url($ticket->attachment) }}"
                                            alt="Lampiran Foto"
                                            class="shadow-2xs h-16 w-16 rounded-md border border-gray-200 object-cover"
                                            :class="removeAttachment ? 'opacity-40' : ''"
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-[11px] text-gray-500">Unggah foto baru di bawah ini jika
                                                ingin mengganti lampiran saat ini.</p>
                                            <label
                                                class="mt-1.5 inline-flex cursor-pointer items-center gap-1.5 text-[11px] font-semibold text-rose-600 hover:text-rose-700">
                                                <input type="checkbox" name="remove_attachment" value="1"
                                                    x-model="removeAttachment"
                                                    class="rounded border-gray-300 text-rose-600 shadow-sm focus:ring-rose-500">
                                                Hapus / hilangkan gambar ini
                                            </label>
                                        </div>
                                    </div>
                                @endif
                                    ...
                                    <div class="space-y-2 text-center">
                                        ...
                                        <div class="flex items-center justify-center gap-1 text-xs text-gray-600">
                                            <label for="attachment"
                                                ...
                                                <span
                                                    x-text="fileName ? 'Ganti File Baru' : '{{ $ticket->attachment ? 'Ganti Foto Lampiran' : 'Pilih File Gambar' }}'">Pilih
                                                    File Gambar</span>
                ...
                <div class="space-y-6 lg:sticky lg:top-6">
                    ...
                    <div class="rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm">
                        ...
                        <dl class="mt-3 space-y-3 text-xs">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <dd class="text-right font-semibold text-gray-800" x-text="title ? title : '—'"></dd>
                            </div>
                            ...
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <dd class="font-semibold text-gray-800"
                                    x-text="fileName ? 'Foto baru dipilih' : (removeAttachment ? 'Akan dihapus' : '{{ $ticket->attachment ? 'Foto lama tetap dipakai' : 'Belum ada' }}')">
                                </dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <!-- Catatan pengeditan -->
                    <div class="shadow-2xs flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50/80 p-5 text-amber-900">
                        <div class="mt-0.5 shrink-0 rounded-lg bg-amber-100 p-1.5 text-amber-700">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="text-xs">
                            ...
                            <p class="mt-1 leading-relaxed text-amber-800">
                                Tiket ini saat ini masih berstatus <span
                                    class="font-semibold uppercase text-blue-700">OPEN</span>. Selama teknisi belum
                                mulai menangani tiket ini, Anda bebas memperbarui informasi atau melampirkan foto
                                kendala baru.
                            </p>
                        </div>
                    </div>
